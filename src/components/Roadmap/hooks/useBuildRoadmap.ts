...
const useBuildRoadmap = (roadmapData: Roadmap) => {
  ...
  const generateSemesterNodes = useCallback((): Node[] => {
    const nodes: Node[] = [];
  ...
  const generateCourseNodes = useCallback(
    (semesterNodes: Node[]): Node[] => {
    ...
    [completedCourses, isEdgesHidden, roadmapData.coursesByYearSemester]
  );

  const generateEdges = useCallback(
    (courseNodes: Node[]): Edge[] => {
      const edges: Edge[] = [];
...
export default useBuildRoadmap;
